...
public class Player {
    ...
    public Card getCardByNeed(ArrayList<Card> oppTook, ArrayList<Card> iAintGotItCuh) {
        List<Card> gimmeDat = hand;
        if(gimmeDat.size() <= 4){
            prevCard = gimmeDat.get(0);
            return gimmeDat.get(0);
        }
        else{
            HashMap<Card, Integer> frequencies = new HashMap<>();
            for (int i = 0; i < hand.size() - 1; i++) {
            int count = 1;
            if(oppTook.size() > 4 && iAintGotItCuh.size() > 4){
            for(int k = oppTook.size()-1; k > oppTook.size()-4; k--){
            if(hand.get(i).getRank().equals(oppTook.get(k).getRank())){
            gimmeDat.remove(hand.get(i));
            }
    }
            for(int l = iAintGotItCuh.size() - 1; l > iAintGotItCuh.size() - 4; l--){
            if(hand.get(i).getRank().equals(oppTook.get(l).getRank())){
               gimmeDat.remove(hand.get(i));
               }
          }
}
            for (int j = i + 1; j < gimmeDat.size(); j++) {
              if (gimmeDat.get(i).equals(gimmeDat.get(j))) {  // tallies cards of the same rank
              count++;
          }
            if (!frequencies.containsKey(gimmeDat.get(i))){
            frequencies.put(gimmeDat.get(i), count);
                    }
                }
            }
            int max = 0;
